...
bool getStruck(int row, int col, Grid<int>& maze) {
    if (!maze.inBounds(row, col)) {
        return true;
    }
    Set<int> barrier {'.', 's', 'X'};
    //east
    bool east_stuck = maze.inBounds(row, col+1) && barrier.contains(maze[row][col+1]);
    //west
    bool west_stuck = maze.inBounds(row, col-1) && barrier.contains(maze[row][col-1]);
    //north
    bool north_stuck = maze.inBounds(row-1, col) && barrier.contains(maze[row-1][col]);
    //south
    bool south_stuck = maze.inBounds(row+1, col) && barrier.contains(maze[row+1][col]);
    return east_stuck && west_stuck && north_stuck && south_stuck;
}

bool solveMazeRecursiveHelper(int row, int col, Grid<int>& maze) {
    if (maze[row][col] == 'F') {
        printMaze(maze);
        return true;
    } else if (getStruck(row, col, maze)) {
        // cout << "stuck at" <<  row << " " << col << endl;
        maze[row][col] = ' ';
        return false;
    } else {
        //north can move? 
        if (maze[row-1][col] == ' ' || maze[row-1][col] == 'F') {
            //if so: choose exlpore unchoose
            // cout << "try north" << endl;
            if (maze[row-1][col] == ' ') {
                maze[row-1][col] = '.';
                if (solveMazeRecursiveHelper(row-1, col, maze))
                    return true;
                maze[row-1][col] = ' ';
            } else {
                solveMazeRecursiveHelper(row-1, col, maze);
                return true;
            }
        }
        ...
        if (maze[row][col+1] == ' ' || maze[row][col+1] == 'F') {
            ...
            if(maze[row][col+1] == ' ') {
                maze[row][col+1] = '.';
                if (solveMazeRecursiveHelper(row, col+1, maze))
                    return true;
                maze[row][col+1] = ' ';
            } else {
                solveMazeRecursiveHelper(row, col+1, maze);
                return true;
            }
        }
...
void solveMazeRecursive(Grid<int> &maze) {
    ...
    solveMazeRecursiveHelper(1, 1, maze);
}
